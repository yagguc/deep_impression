...
class ConvolutionBlock(chainer.Chain):
    def __init__(self, in_channels, out_channels):
        super(ConvolutionBlock, self).__init__(
            conv = chainer.links.Convolution2D(in_channels, out_channels, (1, 49), (1, 4), (0, 24), initialW = chainer.initializers.HeNormal()),
...
class ResidualBlock(chainer.Chain):
    def __init__(self, in_channels, out_channels):
        super(ResidualBlock, self).__init__(
            res_branch2a = chainer.links.Convolution2D(in_channels, out_channels, (1, 9), pad = (0, 4), initialW = chainer.initializers.HeNormal()),
            bn_branch2a = chainer.links.BatchNormalization(out_channels),
            res_branch2b = chainer.links.Convolution2D(out_channels, out_channels, (1, 9), pad = (0, 4), initialW = chainer.initializers.HeNormal()),
...
class ResidualBlockB(chainer.Chain):
    def __init__(self, in_channels, out_channels):
        super(ResidualBlockB, self).__init__(
            res_branch1 = chainer.links.Convolution2D(in_channels, out_channels, (1, 1), (1, 4), initialW = chainer.initializers.HeNormal()),
            bn_branch1 = chainer.links.BatchNormalization(out_channels),
            res_branch2a = chainer.links.Convolution2D(in_channels, out_channels, (1, 9), (1, 4), (0, 4), initialW = chainer.initializers.HeNormal()),
            bn_branch2a = chainer.links.BatchNormalization(out_channels),
            res_branch2b = chainer.links.Convolution2D(out_channels, out_channels, (1, 9), pad = (0, 4), initialW = chainer.initializers.HeNormal()),
            bn_branch2b = chainer.links.BatchNormalization(out_channels)
        )
